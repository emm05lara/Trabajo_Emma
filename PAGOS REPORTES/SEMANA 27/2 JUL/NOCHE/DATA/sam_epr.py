...
# --- 1. CARGA DE DATOS ---
df = pd.read_excel('sam.xlsx', sheet_name='Pagos Despacho 59090')

# --- 2. LIMPIEZA DE FECHAS Y FILTRO DE SEMANA 26 ---
df['Fecha'] = pd.to_datetime(df['Fecha Recepción'], format='%d/%m/%Y %H:%M:%S').dt.date
fecha_inicio = datetime.strptime('30/06/2025', '%d/%m/%Y').date()
fecha_fin = datetime.strptime('06/07/2025', '%d/%m/%Y').date()
...
cuentas_por_segmento = {
    'Extrajudicial - 0 a 28 semanas de atraso': 3928,
    'Extrajudicial - 29 a 39 semanas de atraso': 4199,
    'Extrajudicial - 40 a 55 semanas de atraso': 6975,
    'Mas 55 semanas de atraso': 15606
}

epr_ideal = {
    'Extrajudicial - 0 a 28 semanas de atraso': 222,
    'Extrajudicial - 29 a 39 semanas de atraso': 119,
    'Extrajudicial - 40 a 55 semanas de atraso': 59,
    'Mas 55 semanas de atraso': 46
}
...
for segmento in cuentas_por_segmento.keys():
    ...
    fig1.update_traces(textposition="top center", line=dict(color='blue', width=3), marker=dict(size=10))
    ...
    fig2.update_traces(textposition="top center", line=dict(color='green', width=3), marker=dict(size=10))
